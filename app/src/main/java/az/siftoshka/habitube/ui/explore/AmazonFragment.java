...
import az.siftoshka.habitube.presentation.explore.AmazonPresenter;
import az.siftoshka.habitube.presentation.explore.AmazonView;
...
public class AmazonFragment extends MvpAppCompatFragment implements AmazonView {

    @InjectPresenter AmazonPresenter discoverPresenter;
    ...
    @ProvidePresenter
    AmazonPresenter discoverPresenter() {
        return Toothpick.openScope(Constants.DI.APP_SCOPE).getInstance(AmazonPresenter.class);
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (bundle != null) {
            index = bundle.getInt("Discover-AP");
            if (index == 0) discoverPresenter.showAmazonPopular(1);
            if (index == 1) discoverPresenter.showAmazonBest(1);
            if (index == 2) discoverPresenter.showAmazonNew(1);
        }
    }
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                try {
                    if (!recyclerView.canScrollVertically(1) && newState == RecyclerView.SCROLL_STATE_IDLE) {
                        discoverPresenter.showMoreAmazonPopular(page);
                        page++;
                    }
    ...
    private void paginateBest() {
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                try {
                    if (!recyclerView.canScrollVertically(1) && newState == RecyclerView.SCROLL_STATE_IDLE) {
                        discoverPresenter.showMoreAmazonBest(page);
                        page++;
                    }
                } catch (Exception ignored) {}
            }
    ...
    private void paginateNew() {
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                try {
                    if (!recyclerView.canScrollVertically(1) && newState == RecyclerView.SCROLL_STATE_IDLE) {
                        discoverPresenter.showMoreAmazonNew(page);
                        page++;
                    }
                } catch (Exception ignored) {}
            }
