...
import az.siftoshka.habitube.presentation.explore.DisneyPresenter;
import az.siftoshka.habitube.presentation.explore.DisneyView;
...
public class DisneyFragment extends MvpAppCompatFragment implements DisneyView {

    @InjectPresenter DisneyPresenter discoverPresenter;
    ...
    @ProvidePresenter
    DisneyPresenter discoverPresenter() {
        return Toothpick.openScope(Constants.DI.APP_SCOPE).getInstance(DisneyPresenter.class);
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (bundle != null) {
            index = bundle.getInt("Discover-D");
            if (index == 0) discoverPresenter.showDisneyPopular(1);
            if (index == 1) discoverPresenter.showDisneyBest(1);
            if (index == 2) discoverPresenter.showDisneyNew(1);
        }
    }
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                try {
                    if (!recyclerView.canScrollVertically(1) && newState == RecyclerView.SCROLL_STATE_IDLE) {
                        discoverPresenter.showMoreDisneyPopular(page);
                        page++;
                    }
    ...
    private void paginateBest() {
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                try {
                    if (!recyclerView.canScrollVertically(1) && newState == RecyclerView.SCROLL_STATE_IDLE) {
                        discoverPresenter.showMoreDisneyBest(page);
                        page++;
                    }
                } catch (Exception ignored) {}
            }
    ...
    private void paginateNew() {
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                try {
                    if (!recyclerView.canScrollVertically(1) && newState == RecyclerView.SCROLL_STATE_IDLE) {
                        discoverPresenter.showMoreDisneyNew(page);
                        page++;
                    }
                } catch (Exception ignored) {}
            }
